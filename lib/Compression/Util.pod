...
Nothing is exported by default: every function below is available on demand, either individually or via the C<:all> tag (see L</EXPORT>). This makes it straightforward to build compressors ranging from a couple of lines that reuse a full pipeline (L</HIGH-LEVEL FUNCTIONS>), down to hand-rolled formats assembled from individual transforms and entropy coders (L</LOW-LEVEL FUNCTIONS> and L</MEDIUM-LEVEL FUNCTIONS>).
...
=head2 CHOOSING A FUNCTION

If you just want to shrink some data and don't care how, reach for a L</HIGH-LEVEL FUNCTIONS> pair such as C<gzip_compress()> / C<gzip_decompress()> (a real, interoperable GZIP stream) or C<bwt_compress()> / C<bwt_decompress()> (a lighter, module-specific container). If you want to design your own container format out of individual building blocks -- for example, applying Huffman coding to the output of your own transform -- use the L</MEDIUM-LEVEL FUNCTIONS>. If you are implementing a specific bit-exact format (such as DEFLATE) or need direct access to bit/byte I/O, transforms, or entropy coders in isolation, use the L</LOW-LEVEL FUNCTIONS>.
...
A bitstring is a string containing only the characters C<1> and C<0> (not packed bits). Many low-level encoders return a bitstring; pass it through C<pack('B*', $bitstring)> to get a packed binary string, and C<unpack('B*', $string)> to go the other way.
...
A string means a binary (non-UTF*) string. Any file-handle used to read or write such a string must have its encoding layer set to C<:raw> (see L</filehandle> below).

=head3 symbols

An array of symbols means an array-ref of non-negative integer values. Symbols are not restricted to the C<[0, 255]> byte range unless a specific function says otherwise (for example C<encode_alphabet_256()>); this is what makes the "symbolic" variants of many functions (C<bwt_encode_symbolic()>, C<lzss_encode_symbolic()>, C<lz77_encode_symbolic()>, etc.) useful for compressing sequences of large integers rather than bytes, such as intermediate output from another codec.

=head3 alphabet

An alphabet is a sorted array-ref of the distinct symbols that occur in a sequence, with no repeats. Several functions (such as C<mtf_encode()> and C<adaptive_ac_encode()>) return or accept an alphabet alongside the encoded data.

=head3 filehandle

A filehandle is denoted by C<$fh>. The encoding of file-handles must be set to C<:raw> (i.e. C<open(my $fh, '<:raw', $path)>), so that bytes are read and written without any character-encoding or line-ending translation.

=head3 entropy coding sub

Many of the higher-level functions accept an optional code-ref parameter (commonly named C<$entropy_sub>) that plugs in the final entropy-coding stage of a pipeline, letting you swap Huffman coding for Arithmetic Coding (or a custom coder) without re-implementing the surrounding pipeline. See L</create_huffman_entry> and L</create_ac_entry> for the two coders built into this module, and L</Combining LZSS + MRL compression> for a usage example.
...
=item * B<String or symbols.> Functions documented as accepting C<\@symbols> will also accept a plain string in its place (it is converted internally via C<string2symbols()>). Functions with a C<_symbolic> counterpart (e.g. C<bwt_encode()> / C<bwt_encode_symbolic()>) exist because the plain version is optimized for byte strings, while the C<_symbolic> version works with arbitrary non-negative integers and has no upper bound on symbol values.

=item * B<Filehandle or string, transparently.> Any decoder that takes C<$fh> as its first argument will also accept a plain packed binary string; internally it opens an in-memory filehandle over the string (C<open($fh2, '<:raw', \$string)>) and recurses. This means C<lzss_decompress($string)> and C<lzss_decompress($fh)> both work, and the two forms can be mixed freely when chaining decoders.

=item * B<Streaming decompression.> Because decoders read only as much as they need from C<$fh>, several compressed blocks can be concatenated back-to-back in a single file and decoded one at a time in a loop (see the L</SYNOPSIS>), without needing to record each block's length up front.

=item * B<Package variables are dynamically scoped.> The C<$LZ_*> tuning variables (L</PACKAGE VARIABLES> below) are read at the time an LZ-family encoder runs. Use C<local> to override them for the duration of a single call, so that concurrent or later calls elsewhere in your program are unaffected:
...
When set to a true value, many functions print diagnostic information (block sizes, header fields, checksums, and so on) to C<STDERR>. Useful when debugging a hand-assembled pipeline or inspecting third-party GZIP/ZLIB/LZ4 streams being decoded.

By default, C<$VERBOSE> is set to C<0>.
...
=head2 $LZ_MAX_CHAIN_WIDTH

The value of C<$LZ_MAX_CHAIN_WIDTH> controls the number of positions to store from each match. A larger value may result in better compression, finding longer matches, at the expense of speed.

By default, C<$LZ_MAX_CHAIN_WIDTH> is set to C<1024>.

B<NOTE:> the function C<lzss_encode_fast()> will ignore this value, always using a value of C<1>.

=head2 CHOOSING AN LZSS ENCODER

The module ships three LZ-parsing engines with different speed/ratio trade-offs, all of which produce output consumable by the same C<lzss_decode()> / C<lzss_decode_symbolic()>:
...
Any of these can be plugged into the higher-level compressors (C<lz77_compress()>, C<lzss_compress()>, C<lzb_compress()>, C<lz4_compress()>, C<gzip_compress()>, C<zlib_compress()>) via their optional C<$lzss_encoding_sub> argument, so the trade-off can be tuned per call without touching the rest of the pipeline.
...
Ready-made compressors and decompressors, each wrapping a full pipeline of transforms and an entropy coder. Start here unless you need to design your own format.
      ...
      detect_format($fh)                   # Sniffs the magic bytes of a compressed blob
      decompress_auto($fh)                 # Decompresses a blob produced by GZIP, ZLIB, BZIP2 or LZ4

      best_compress($data)                 # Compresses $data using several methods, optimizing for best size
      best_decompress($data)               # Inverse of the above method

=head1 MEDIUM-LEVEL FUNCTIONS

Individual transforms and entropy coders, meant to be composed together into your own pipeline (as the L</HIGH-LEVEL FUNCTIONS> do internally).

      deltas(\@ints)                       # Computes the differences between integers
      accumulate(\@deltas)                 # Inverse of the above method

      delta_encode(\@ints)                 # Deltas+RLE+Elias delta encoding of an array-ref of integers
      ...
      elias_delta_encode(\@symbols)        # Elias Delta coding method of an array-ref of symbols
      elias_delta_decode($fh)              # Inverse of the above method
      ...
      golomb_rice_encode(\@symbols)        # Encodes a sequence of symbols using Golomb-Rice coding
      golomb_rice_decode($fh)              # Inverse of the above method

      leb128_encode(\@symbols)             # Encodes a sequence of symbols using unsigned LEB128 (Little Endian Base 128)
      leb128_decode($fh)                   # Inverse of the above method
      ...
      shannon_entropy(\@symbols)           # Computes the (zeroth-order) Shannon entropy of a sequence of symbols
...
Unlike C<create_ac_entry()>, no frequency table needs to be stored in the block: the model adapts to the data as it is decoded, which can save space on short or skewed inputs at the cost of somewhat slower encoding/decoding.
...
High-level function that performs byte-oriented LZSS compression, inspired by LZ4. Unlike C<lz4_compress()>, this is a self-contained format specific to this module, not an interoperable LZ4 stream.
...
Valid Bzip2 compressor, given a string or an input file-handle. The output can be decompressed by any standard C<bzip2> tool, not just by C<bzip2_decompress()>.
...
Valid Bzip2 decompressor, given a string or an input file-handle. Understands streams produced by the standard C<bzip2> tool as well as by C<bzip2_compress()>.
...
Valid GZIP compressor (RFC 1952), given a string or an input file-handle. The output is a standard C<.gz> stream, decodable with C<gzip -d>, C<zcat>, or any GZIP-compatible library.
...
Valid GZIP decompressor (RFC 1952), given a string or an input file-handle. Understands streams produced by the standard C<gzip> tool as well as by C<gzip_compress()>, including multiple concatenated members.
...
A fast compression method (no BWT stage, so it is cheaper but generally less effective than C<bwt_compress()>), using the following pipeline:
...
It accepts either a string or an arbitrary array-ref of non-negative integer values as input; C<mrl_compress()> and C<mrl_compress_symbolic()> are, in fact, the same function under two names.
...
Inverse of C<mrl_compress()> and C<mrl_compress_symbolic()>.

=head2 detect_format

    my $format = detect_format($string);
    my $format = detect_format($fh);

Sniffs the magic bytes of a compressed blob (or the next few bytes of a filehandle) and returns one of C<"gzip">, C<"zlib">, C<"bzip2">, C<"lz4">, or C<undef> if none of the four is recognized.

Given a filehandle, C<detect_format()> peeks at the first 4 bytes and seeks back, leaving the filehandle's position unchanged -- so it is safe to call before handing the same filehandle to C<gzip_decompress()>, C<zlib_decompress()>, and so on (this is exactly what C<decompress_auto()> does internally). The filehandle must support C<seek()> (a real file or an in-memory string filehandle both do; a pipe or socket does not).

Detection for gzip, bzip2 and lz4 is exact (each format has a fixed, unambiguous magic number). Zlib streams have no dedicated magic number by design of the format, so detection there is a validity check on the header's checksum bits (RFC 1950, section 2.2) -- reliable in practice, but with an intrinsic (very small, roughly 1-in-500) false-positive rate on arbitrary non-zlib data, which is a property of the zlib format itself rather than of this function.

Note this only recognizes the four self-describing container formats above; it has no way to distinguish this module's own C<bwt_compress()>/C<mrl_compress()> output (which carries no format signature) from arbitrary binary data. See C<best_compress()> below if you need to auto-detect between this module's own compression methods.

=cut

=head2 decompress_auto

    my $data = decompress_auto($string);
    my $data = decompress_auto($fh);

Decompresses a blob produced by (real) C<gzip>, C<zlib>, C<bzip2>, or C<lz4> -- or by this module's own C<gzip_compress()>/C<zlib_compress()>/C<bzip2_compress()>/C<lz4_compress()> -- without the caller needing to know in advance which of the four it is. Internally calls C<detect_format()> and dispatches to the matching C<*_decompress()> function; dies if the format isn't recognized.

Example:

    # Anywhere upstream, some file may have been compressed with any tool:
    my $data = decompress_auto($fh);
...
=head2 best_compress
    ...
    my $string = best_compress($data);
    my $string = best_compress($data, \@methods);

Compresses C<$data> using several methods and keeps whichever produced the smallest output, prefixed with a single-character tag identifying which method won (so C<best_decompress()> knows how to reverse it).

By default, three of this module's own high-level compressors are tried: C<gzip_compress()> (tag C<"G">), C<bwt_compress()> (tag C<"B">), and C<mrl_compress()> (tag C<"M">) -- deliberately three different algorithm families (LZ77/LZSS, BWT+MTF, MTF-only), so that at least one tends to do reasonably well regardless of what kind of redundancy the input has. This costs roughly the sum of all three methods' compression time; it is meant for cases where compression happens once (or offline) and is read many times, not for a hot path where compression speed matters as much as ratio.

The default set can be overridden with C<\@methods>, an array-ref of C<[$tag, $compress_sub]> pairs, where C<$tag> is a single character and C<$compress_sub> is a code-ref taking the same single argument C<best_compress()> itself received (a string, or whatever C<$compress_sub> accepts). If you do this, pass a matching tag-to-decompressor mapping to C<best_decompress()> (see below), since its own default set only understands C<"G">/C<"B">/C<"M">.

Example:

    my $enc = best_compress($data);
    say best_decompress($enc) eq $data;    # true

    # Restrict the race to two fast methods:
    my $enc2 = best_compress($data, [
        ['M', \&mrl_compress],
        ['L', \&lzss_compress],
    ]);

=head2 best_decompress

    my $data = best_decompress($fh);
    my $data = best_decompress($string);
    my $data = best_decompress($fh, \%methods);

Inverse of C<best_compress()>. Reads the single-character method tag and dispatches to the corresponding decompressor.

The default C<%methods> mapping:
...
matches C<best_compress()>'s default set.

If C<best_compress()> was called with a custom C<\@methods> list, pass the corresponding C<<< {tag => $decompress_sub} >>> hash-ref here as well.
...
=head2 shannon_entropy

    my $bits_per_symbol = shannon_entropy(\@symbols);

Computes the (zeroth-order) Shannon entropy of a sequence of symbols, in bits per symbol, based on the empirical frequency of each distinct symbol in C<\@symbols> (via C<frequencies()>).

This is the information-theoretic lower bound on the average code length any symbol-by-symbol entropy coder (Huffman coding, arithmetic coding) could achieve on this data, ignoring any structure a transform like BWT or LZ parsing might expose beforehand. It's useful for deciding whether an entropy-coding stage is worth its overhead at all, for comparing how much a preprocessing transform (MTF, delta, BWT) has "flattened" the distribution, or for estimating a theoretical best-case compressed size before actually running a coder.

Returns C<0> for an empty array-ref or when every symbol is identical.

Example:

    shannon_entropy([0,1,0,1,0,1]);         # 1.0 -- one fair coin flip per symbol
    shannon_entropy([5,5,5,5,5]);           # 0.0 -- no information at all
    shannon_entropy(string2symbols($text)); # bits/byte of the raw text

    # Multiply by the symbol count for a total-bits estimate:
    my $symbols   = string2symbols($data);
    my $est_bytes = shannon_entropy($symbols) * scalar(@$symbols) / 8;

=head2 deltas

    my $deltas = deltas(\@integers);

Computes the differences between consecutive integers, returning an array-ref, where the first delta is relative to C<0>. For example:
...
Inverse of C<deltas()>. For example:
...
Encodes a sequence of integers (including negative integers) using Deltas + Run-length + Elias delta coding, returning a binary string.

Delta encoding calculates the difference between consecutive integers in the sequence and encodes these differences using Elias delta coding. When it's beneficial, runs of identical symbols are collapsed with RLE.
...
=head2 elias_delta_encode

    my $string = elias_delta_encode(\@symbols);

Encodes a sequence of non-negative integers using Elias Delta coding, returning a binary string. Elias Delta coding encodes the bit-length of each value using Elias Gamma coding, then appends the value's remaining bits directly. It sits between Elias Gamma and Elias Omega: tighter than Gamma on large values, without Omega's fully recursive length-of-length encoding.

=head2 elias_delta_decode

    # Given a file-handle
    my $symbols = elias_delta_decode($fh);

    # Given a binary string
    my $symbols = elias_delta_decode($string);

Inverse of C<elias_delta_encode()>.
...
This method is particularly effective in encoding a sequence of integers that are in ascending order or have roughly the same size in binary (this is why it is used as the default entropy coder for LZW codes in C<lzw_compress()>, since LZW dictionary indices grow roughly monotonically).
...
=head2 golomb_rice_encode

    my $string = golomb_rice_encode(\@integers);
    my $string = golomb_rice_encode(\@integers, $k);

Encodes a sequence of non-negative integers using Golomb-Rice coding, returning a binary string.

Golomb-Rice coding splits each integer C<n> into a quotient C<n E<gt>E<gt> k> (stored in unary) and a remainder C<n & (2**k - 1)> (stored in C<$k> fixed bits). It is near-optimal for geometrically-distributed data -- residuals from a predictor, run lengths, gaps between sorted values -- and is the entropy coder used by FLAC and many other lossless audio/image formats for exactly that reason.

If C<$k> is omitted, it is chosen automatically from the mean of C<\@integers> using the standard approximation for geometric sources. Passing C<$k> explicitly skips this estimate, which is useful when you already know (or want to fix) the distribution's scale, or when encoding many small sequences drawn from the same distribution and want a single shared C<$k> instead of paying for a per-call estimate.

Unlike C<elias_gamma_encode()>/C<elias_omega_encode()>, which are parameter-free but assume small values are far more likely than large ones, Golomb-Rice coding is tunable to the actual scale of the data, and generally compresses geometric/Laplacian-ish data more tightly once C<$k> is well chosen.

Example:

    my @residuals = (0, 1, 0, 2, 0, 0, 1, 3, 0, 1, 0, 0, 2, 1, 0);
    my $enc = golomb_rice_encode(\@residuals);
    golomb_rice_decode($enc);    # \@residuals

=head2 golomb_rice_decode

    my $integers = golomb_rice_decode($fh);
    my $integers = golomb_rice_decode($string);

Inverse of C<golomb_rice_encode()>. The array length and the C<$k> parameter used at encoding time are both stored in the stream (via C<elias_omega_encode()>), so nothing besides the encoded string/filehandle needs to be supplied.

=head2 leb128_encode

    my $string = leb128_encode(\@integers);

Encodes a sequence of non-negative integers using unsigned LEB128 (Little Endian Base 128), the same variable-length byte encoding used by protobuf, WebAssembly, DWARF, and the Git packfile format. Each integer is split into 7-bit groups, least-significant group first; every byte but the last has its high bit set as a continuation marker.

Being byte-aligned rather than bit-aligned, LEB128 is both faster to decode than any of the bit-level codes in this module (C<fibonacci_encode()>, C<elias_gamma_encode()>, C<golomb_rice_encode()>, ...) and produces output compatible with other LEB128 implementations -- useful when the priority is decode speed or interop rather than the tightest possible ratio. For small values it is also reasonably compact: any value below 128 costs exactly one byte.

As with C<fibonacci_encode()> and the Elias coders, the array length is stored as the first encoded value, so C<leb128_decode()> is fully self-contained.

Example:

    my $enc = leb128_encode([0, 1, 127, 128, 300, 16384]);
    leb128_decode($enc);    # [0, 1, 127, 128, 300, 16384]

=head2 leb128_decode

    my $integers = leb128_decode($fh);
    my $integers = leb128_decode($string);

Inverse of C<leb128_encode()>.
...
This method is particularly effective in encoding a sequence of moderately large random integers, such as the list of distances returned by C<lzss_encode()> (which is exactly how C<lz77_compress()> uses it internally).
...
By default, C<$lookahead_len> is C<128> (see C<bwt_sort()>).
...
Performs Move-To-Front (MTF) encoding on a sequence of symbols (also accepts a plain string in place of C<\@symbols>).
...
Encode a sorted alphabet of symbols into a binary string. Use C<encode_alphabet_256()> when you know every symbol fits in a byte (for instance, the alphabet produced by C<mtf_encode()> on ordinary text); it produces a more compact header than the general-purpose C<encode_alphabet()>.
...
Decodes an encoded alphabet, given a file-handle or a binary string, returning an array-ref of symbols. Inverse of C<encode_alphabet()> and C<encode_alphabet_256()>, respectively.
...
This function efficiently encodes runs of zeros, but also increments each symbol by C<1> (so that C<0> is freed up to serve purely as a run marker in the output alphabet -- useful right after C<mtf_encode()>, whose output is dominated by zeros).
...
The function returns two values: C<$bitstring>, which is a string of 1s and 0s, and C<$freq>, representing the frequency table used for encoding -- this table must be kept (or otherwise reconstructed) in order to decode the data with C<ac_decode()>, since it is not embedded in C<$bitstring>. (C<create_ac_entry()> takes care of storing it for you.)
...
The function returns two values: C<$bitstring>, which is a string of 1s and 0s, and C<$alphabet>, which is an array-ref of distinct sorted symbols. Unlike C<ac_encode()>, no per-symbol frequencies need to be stored -- only the alphabet -- because the model adapts as it goes.
...
Compute the Adler-32 checksum of a given string, as used by ZLIB. Like C<crc32()>, it can be computed incrementally by passing the running checksum as the second argument; note that its identity value is C<1>, not C<0>.
...
The function stores the extra bits inside the C<$buffer>, reading one character at a time from the file-handle. Pass the same C<\$buffer> scalar-ref on every call so left-over bits from a partially consumed byte carry over correctly to the next read.
...
Note: to turn a bitstring back into a packed binary string (rather than reading it from a file-handle with C<bits2int()>), use C<pack('B*', $bitstring)>.
...
Convert a non-negative integer to a byte-string of width C<$size>, in MSB order. For example, C<int2bytes(1, 4)> returns the 4-byte big-endian encoding of C<1>.

=head2 int2bytes_lsb

    my $string = int2bytes_lsb($symbol, $size);

Convert a non-negative integer to a byte-string of width C<$size>, in LSB order (little-endian).
...
Returns an array-ref of code points, given a string. Equivalent to C<[unpack('C*', $string)]>.

=head2 symbols2string

    my $string = symbols2string(\@symbols)

Returns a string, given an array-ref of code points. Equivalent to C<pack('C*', @$symbols)>. Inverse of C<string2symbols()>.

=head2 read_null_terminated

    my $string = read_null_terminated($fh)

Read a string from file-handle C<$fh> that ends with a NULL character ("\0"). The terminator itself is consumed but not included in the returned string.
...
It takes two parameters: C<$string>, which is the input string to be transformed, and C<$lookahead_len> (optional), representing the length of look-ahead during sorting, which trades memory for speed (O(n * C<$lookahead_len>) space). By default, C<$lookahead_len> is C<128>.
...
Low-level function that sorts the rotations of a sequence of symbolic elements using the Burrows-Wheeler Transform (BWT) algorithm. Unlike C<bwt_sort()>, this uses O(n) space with no look-ahead parameter, and is somewhat slower for large inputs.

It takes a single parameter C<\@symbols>, which represents the input sequence of symbolic elements. The function returns an array-ref of indices.
...
It takes a single parameter, C<\%freq>, representing the hash table where keys are symbols, and values are their corresponding frequencies (such as the hash-ref returned by C<frequencies()>).
...
Low-level function that constructs Huffman prefix codes, given an array-ref of symbols. Equivalent to C<huffman_from_freq(frequencies(\@symbols))>.
...
Low-level function that constructs a dictionary of canonical prefix codes as defined in RFC 1951 (Section 3.2.2), given an array-ref of code lengths or a hash-ref of C<< symbol => length >> values.

It takes a single parameter, C<\@code_lengths>, where entry C<$i> in the array corresponds to the code length for symbol C<$i> (a length of C<0> means the symbol is unused).

Similarly, when a hash-ref table is given, C<\%code_lengths>, keys are the symbols and values are the code lengths. This variant is useful for large symbols, where an array indexed by symbol value would be wastefully sparse.
...
Low-level function that performs Huffman encoding on a sequence of symbols using a provided dictionary, returned by C<huffman_from_freq()>, C<huffman_from_symbols()>, or C<huffman_from_code_lengths()>.
...
Putting the last few functions together, without using C<create_huffman_entry()>'s bookkeeping:
...
The output can be decoded with C<lz77_decode()> and C<lz77_decode_symbolic()>, respectively. C<lz77_encode_symbolic()> is currently the same underlying function as C<lz77_encode()>, since C<lz77_encode()> already dispatches on whether it was given a string or an array-ref.

Ignores C<$LZ_MAX_LEN>, always using unlimited match lengths (see L</$LZ_MAX_LEN>).
...
Low-level function that applies the LZSS (Lempel-Ziv-Storer-Szymanski) algorithm on the provided data. See L</CHOOSING AN LZSS ENCODER> for how this compares to C<lzss_encode_fast()>.
...
Any key omitted from C<%params> falls back to the corresponding package variable (see L</PACKAGE VARIABLES>).
...
Inverse of C<lzss_encode()> and C<lzss_encode_fast()> (both produce output in the same C<($literals, $distances, $lengths)> shape).
...
Low-level function that encodes the results returned by C<lzss_encode()> or C<lzss_encode_fast()>, using a DEFLATE-like approach, combined with Huffman coding by default (or the given C<$entropy_sub>).
...
Low-level function that returns the index inside the DEFLATE tables (as returned by C<make_deflate_tables()>) for a given value.
...
Given an input filehandle, a bitstring buffer and a search window, it extracts the next DEFLATE block, dispatching automatically to C<deflate_extract_block_type_0()>, C<deflate_extract_block_type_1()>, or C<deflate_extract_block_type_2()> based on the block-type bits. The next two bits in the input file-handle (or in the bitstring buffer) must contain the block-type number.

This is the function C<gzip_decompress()> and C<zlib_decompress()> use internally to walk a stream one block at a time; C<$search_window> accumulates the last 32 KiB of decoded output across calls, as required for resolving back-references near a block boundary.
...
The package variables (C<$VERBOSE>, C<$LZ_MIN_LEN>, C<$LZ_MAX_LEN>, C<$LZ_MAX_DIST>, C<$LZ_MAX_CHAIN_LEN>, C<$LZ_MAX_CHAIN_WIDTH>) are B<not> included in C<:all>; import them explicitly by name if needed (see L</PACKAGE VARIABLES>).
...
=head2 Compressing a list of integers directly (no string involved):
...
=head2 Choosing an entropy coder based on input size:

Adaptive Arithmetic Coding avoids storing an explicit frequency table, which tends to pay off on short inputs, while plain Huffman coding (via C<create_huffman_entry()>) is typically faster and does just as well once the input is large enough to amortize the table's size:
...
=head2 Tuning LZ parsing for speed vs. ratio:
...
=item * L<Compress::Zlib>, L<IO::Compress::Gzip>, L<IO::Compress::Bzip2> -- bindings to the corresponding C libraries, generally much faster than this pure-Perl implementation for production use.

=item * L<Compress::Raw::Zlib> -- low-level bindings, if only speed matters and not portability.

=back

Use B<Compression::Util> when you want a pure-Perl implementation with no XS/C dependencies, when you want to inspect or hand-tune every stage of the pipeline, or for learning how these algorithms work.
