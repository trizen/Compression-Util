...
The default C<%methods> mapping (C<< G => \&gzip_decompress, B => \&bwt_decompress, M => \&mrl_decompress >>) matches C<best_compress()>'s default set. If C<best_compress()> was called with a custom C<\@methods> list, pass the corresponding C<{tag => $decompress_sub}> hash-ref here as well.
